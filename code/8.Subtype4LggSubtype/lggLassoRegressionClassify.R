...
difExpQvalue <- readRDS(file = 'D:/CancerNeuroscience/Github/data/lggCluster2DiffExpGene.rds')
...
lggCluster <- hcPearsonWardAverCluster[[5]]$consensusClass %>% as.data.frame() %>% rownames_to_column(var='SAMPLE_BARCODE')
colnames(lggCluster)[2] <- 'Clusters'

lggCluster <- lggCluster %>% left_join(tcgaPanCanSamples[, c('SAMPLE_BARCODE', 'DISEASE')], by = 'SAMPLE_BARCODE') %>% 
  mutate(Clusters = paste0('Cluster', Clusters)) %>% column_to_rownames(var = 'SAMPLE_BARCODE') %>% subset(DISEASE == 'LGG')
...
lggCluster <- lggCluster %>% select(Clusters) %>% mutate(Clusters = ifelse(Clusters == 'Cluster4', 1, 0)) %>% as.matrix()
...
pdf('D:/CancerNeuroscience/Github/result/section5/lgglike/cv_curve_auc.pdf')
plot(cv.fit)
dev.off()
...
save(cv.fit, active.k.vals, active.k.vals.1se, file='D:/CancerNeuroscience/Github/data/lgg_lasso_binomial_res.RData')
