...
# read count data
exp.count <- read.csv(file = '/data/GliomaData/CGGA.mRNAseq_693.Read_Counts-genes.20220620.txt', 
                      header = T, sep = '\t', stringsAsFactors = F)

exp.count <- exp.count %>% tibble::column_to_rownames(var = 'gene_name')

# read clinical data
cli.data <- read.csv(file = '/data/GliomaData/CGGA.mRNAseq_693_clinical.20200506.txt', 
                     header = T, sep = '\t', stringsAsFactors = F)
...
saveRDS(sub.cli.data, file='/data/LggRiskScores/plgg_693_risk_score.rds')
