...
# read count data
exp.count <- read.csv(file = 'D:/CancerNeuroscience/data/GliomaData/CGGA.mRNAseq_693.Read_Counts-genes.20220620.txt', 
                      header = T, sep = '\t', stringsAsFactors = F)

exp.count <- exp.count %>% tibble::column_to_rownames(var = 'gene_name')

# read clinical data
cli.data <- read.csv(file = 'D:/CancerNeuroscience/data/GliomaData/CGGA.mRNAseq_693_clinical.20200506.txt', 
                     header = T, sep = '\t', stringsAsFactors = F)
...
SurvivalPlot(survival.data=sub.cli.data[, c('patient_id', 'OS', 'Censor..alive.0..dead.1.')], 
             sample.class=sub.cli.data[, c('patient_id', 'risk.categ')], filename='plgg_693_os.pdf', 
             out.file.path='D:/CancerNeuroscience/Github/result/section5/lgglike/')

saveRDS(sub.cli.data, file='D:/CancerNeuroscience/Github/data/LggRiskScores/plgg_693_risk_score.rds')
...
cli.sig.char <- sub.cli.data %>% select(patient_id, os_time, os, age_categ, gender, Histology, Grade, idh.status, X1p19q.codeletion, 
                                        risk.categ, mgmt.status) # , TMZ.treated, Radio.status
